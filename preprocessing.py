...
class ResamplingTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, use_resampling=True):
        self.use_resampling = use_resampling
        self.nm = NearMiss(sampling_strategy=1, n_jobs=-1)

    def fit(self, X, y=None):
        return self

    def transform(self, X, y=None):
        if self.use_resampling and y is not None:
            X_resampled, y_resampled = self.nm.fit_resample(X, y)
...
preprocessor = Pipeline([
    ...
    ('resampling_preprocessing', ResamplingTransformer(use_resampling=True))
])
